...
export default {
  ...
  mounted() {
    ...
    //   for (var i = 0; i < 2; i++) {
    //     for (var j = 0; j < 2; j++) {
    //       document.getElementById("d" + chart[i][j]).style.backgroundPositionX =
    //         j * (100 / (2 - 1)) + "%";
    //       document.getElementById("d" + chart[i][j]).style.backgroundPositionY =
    //         i * (100 / (2 - 1)) + "%";
    //     }
    //   }
  ...
  methods: {
    ...
    clickChange(id) {
      if (this.change_flag == false) {
        ...
        let func = require("../assets/js/puzzle.js");
        func.move(id, this.firstId, 2, this.$store.state.first_flag);
        // this.move(id, this.firstId, 2, this.$store.state.first_flag);
        console.log("changed");
    ...
    move(id, firstId, num, _flag) {
      console.log("changing");
      // 找到存储的数据为firstId的位置
      for (var m = 0; m < num; m++) {
        for (var n = 0; n < num; n++)
          if (this.chart[m][n] == firstId) {
      ...
      for (var i = 0; i < num; i++) {
        for (var j = 0; j < num; j++)
          if (this.chart[i][j] == id) {
            var id_pos = num * i + j; //通过二维数组的下标计算出存储id的位置序列
            break;
          }
      }

      if (this.$options.methods.canIf(id_pos, first_id_pos, d_direct)) {
        // 可以交换则交换两个拼图块所在位置对应的二维数组处所存储的id
        // 二维数组存储的数据为拼图块div的id
        this.chart[m][n] = id;
        this.chart[i][j] = firstId;
    ...
    canIf(id_pos, first_id_pos, d_direct) {
      var move_flag = false;
