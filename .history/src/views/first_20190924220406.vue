...
export default {
  ...
  methods: {
    moveImage() {
      ...
      let dx, dy, newLeft, newTop, startX, startY;
      ...
      for (var i = 0; i < pics.length; i++) {
        pics[i].addEventListener("touchstart", function(e) {
          ...
          startX = this.offsetLeft; //记录当前初始状态水平发生改变时的位置
          startY = this.offsetTop; //offsetTop等取得的值与this.style.left获取的值区别在于前者不带px,后者带px
          console.log(e.targetTouches[0]);
          this.style.transition = "none";
        });

        pics[i].addEventListener("touchmove", function(e) {
          this.newLeft = e.targetTouches[0].pageX - this.dx; //记录拖拽的水平状态发生改变时的位置
          this.newTop = e.targetTouches[0].pageY - this.dy;

          if (this.newLeft <= -picWidth / 2) {
            //限制边界代码块，拖拽区域不能超出边界的一半
            this.newLeft = -picWidth / 2;
          } else if (this.newLeft >= boxWidth - picWidth / 2) {
            this.newLeft = boxWidth - picWidth / 2;
          }
          if (this.newTop <= -picHeight / 2) {
            this.newTop = -picWidth / 2;
          } else if (this.newTop >= boxHeight - picHeight / 2) {
            this.newTop = boxHeight - picHeight / 2;
          }
          this.style.left = this.newLeft + "px";
          console.log(this.style);
          this.style.top = this.newTop + "px"; //设置目标元素的left,top
        });
