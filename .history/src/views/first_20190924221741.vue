...
export default {
  ...
  methods: {
    moveImage() {
      ...
      let dx, dy, newLeft, newTop, startX, startY, endX, endY;
      ...
      for (var i = 0; i < pics.length; i++) {
        ...
        pics[i].addEventListener("touchmove", function(e) {
          ...
          if (newLeft <= -picWidth / 2) {
            ...
            newLeft = -picWidth / 2;
          } else if (newLeft >= boxWidth - picWidth / 2) {
            newLeft = boxWidth - picWidth / 2;
          }
          if (newTop <= -picHeight / 2) {
            newTop = -picWidth / 2;
          } else if (newTop >= boxHeight - picHeight / 2) {
            newTop = boxHeight - picHeight / 2;
          }
        ...
        pics[i].addEventListener("touchend", function(e) {
          ...
          endX = e.changedTouches[0].pageX - dx;
          endY = e.changedTouches[0].pageY - dy; //记录滑动结束时的位置，与进入元素对比，判断与谁交换
